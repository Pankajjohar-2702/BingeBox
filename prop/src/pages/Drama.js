...
const Drama = () => {
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch('/api/videos?category=drama');
      const videos = await response.json();
      setData(groupVideos(videos));
    } catch (error) {
      console.error('Failed to load drama videos:', error);
    } finally {
      setLoading(false);
    }
  };
      ...
      {showSearchResults ? (
        ...
        <div id="allContentWrapper">
          <div className="banner">
            <h2>Drama Collection</h2>
            <p>Immersive stories that captivate and inspire</p>
          </div>

          {renderSection('🇰🇷 Korean Drama', data.koreanDrama)}
          {renderSection('🇨🇳 Chinese Drama', data.chineseDrama)}
          {renderSection('🎬 Hollywood Drama', data.hollywoodDrama)}
          {renderSection('🎭 Classic Hits', data.classicHits)}
        </div>
      )}
